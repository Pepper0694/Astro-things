...
def flux_cal():
    ...
    wavelength_ir=np.append(wavelength_ir, zeroes)
 ...
 #   ax[0,1].set_title('Sirius_Vis_EXP')
  ...
  #  ax[1,1].plot(y_cal_IR,tot_IR)
   # ax[1,1].set_title('Sirius_IR_EXP')
    ...
    for i in range(len(flux_vis)):
        for j in range(len(y_cal_Vis)): 
            if abs(wavelength_vis[i]-y_cal_Vis[j])<0.2453:
                ...
                good_wavelengths.append(y_cal_Vis[i])
    ...
    array= np.array([good_wavelengths, good_lines_exp_ir, good_lines_theo_ir])
    with open('vegaIR2.pkl', 'wb') as fout:
